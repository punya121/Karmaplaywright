...
/**
 * The live two-browser consultation, as one function so more than one spec can drive it:
 *
 *   tests/live/full-consultation-with-doctor.spec.ts     one random patient
 *   tests/live/live-consultation-gender-age.spec.ts      a man, a woman and a child under five
 *
 * See tests/live/full-consultation-with-doctor.spec.ts for how the two sessions fit
 * together. The patient is the caller's, so the caller decides who is registered.
 */
...
/**
 * Browser 2, held outside the journey so teardown can close it even when the journey
 * fails half way through - an orphaned chromium holding a doctor session open is the one
 * piece of damage a crashed run here could otherwise do.
 */
export type DoctorSession = {
    ...
    cameOnDutyThisRun: boolean;
};

export function createDoctorSession(): DoctorSession {
    return { cameOnDutyThisRun: false };
}

/**
 * Browser 2 first: it is the one holding the doctor's session, and the account allows
 * only a handful of concurrent ones, so it is checked out and logged out rather than just
 * closed. Browser 1 last, so the patient session outlives the doctor's - the consultation
 * is held open for the whole of the doctor's visit to it, which is the point.
 */
export async function closeLiveSessions(page: Page, session: DoctorSession): Promise<void> {
    if (session.page) {
        if (session.cameOnDutyThisRun) {
            await new DoctorHomePage(session.page).checkOut().catch(() => undefined);
        }
        await new LoginPage(session.page).logout().catch(() => undefined);
    }
    ...
    session.cameOnDutyThisRun = false;

    await new LoginPage(page).logout().catch(() => undefined);
}

/**
 * The whole journey: browser 1 registers `patient` and hands the visit over, browser 2
 * signs the doctor in, joins, writes, saves and approves the consultation.
 */
export async function runLiveConsultation(
    page: Page,
    ...
    await runModuleCases([
            ...
            run: async () => {
                ...
                test.info().annotations.push({
                    ...
                    description:
                        ...
                        `${patient.gender}, aged ${patient.age} ${patient.ageUnit}`,
                });
                console.log(
        ...
        },
        {
            ...
            run: async () => {
                ...
                doctorLogin = new LoginPage(session.page);
                doctorHome = new DoctorHomePage(session.page);
                prescriptionForm = new PrescriptionFormPage(session.page);
        ...
        },
        {
            ...
            run: async () => {
                ...
                session.cameOnDutyThisRun = await doctorHome.ensureCheckedIn();

                test.info().annotations.push({
                    type: 'doctor check-in',
                    description: `${doctorUsername} ${
                        session.cameOnDutyThisRun ? 'checked in by this run' : 'was already on duty'
                    }`,
                });
                console.log(
        ...
        },
        {
            ...
            run: async () => {
                const doctor = session.page as Page;
